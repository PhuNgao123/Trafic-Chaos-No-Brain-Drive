...
public class VehicleSpawner : MonoBehaviour
{
    ...
    // Get road Y position at given Z coordinate using raycast
    float GetRoadYAtPosition(float zPos)
    {
        // Raycast downward from high position to find road
        // Use spawner's X position (lane position) for accurate road detection
        Vector3 rayStart = new Vector3(transform.position.x, 50f, zPos);
        RaycastHit hit;
        
        if (Physics.Raycast(rayStart, Vector3.down, out hit, 100f))
        {
            Debug.Log($"[VehicleSpawner] Raycast hit {hit.collider.name} at Y={hit.point.y}, tag={hit.collider.tag}");
            return hit.point.y;
        }
        
        // Fallback: if no hit, use spawner's Y position
        Debug.LogWarning($"[VehicleSpawner] Raycast missed! Using spawner Y={transform.position.y}");
