...
public class VehicleMove : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (_rigidbody != null)
        {
            ...
            // Only freeze rotations, let Y position free so vehicle can fall to ground
            _rigidbody.constraints = RigidbodyConstraints.FreezeRotationX | 
                                    RigidbodyConstraints.FreezeRotationY | 
                                    RigidbodyConstraints.FreezeRotationZ;
            
            Debug.Log($"[VehicleMove] {gameObject.name} initialized - Gravity={_rigidbody.useGravity}, Constraints={_rigidbody.constraints}");
        }
        else
        {
            Debug.LogError($"[VehicleMove] {gameObject.name} is missing Rigidbody component!");
        }
    }
    ...
    void Move()
    {
        ...
        Vector3 newPosition = _rigidbody.position + moveDirection * _currentSpeed * Time.deltaTime;

        // Smooth lane change movement
        newPosition.x = Mathf.MoveTowards(_rigidbody.position.x, _targetX, laneChangeSpeed * Time.deltaTime);

        // Use Rigidbody.MovePosition for physics-based movement
        _rigidbody.MovePosition(newPosition);

        // Mark lane change as complete when reached target
        if (Mathf.Abs(_rigidbody.position.x - _targetX) < 0.05f)
